...
def policy_iteration(env: mdp_env, agent: mdp_agent, max_iter = 1000) -> np.ndarray:
    ...
    for i in range(max_iter):
        for state in range(len(env.states)):
            sum = 0
            for nextState in range(len(env.states)):
                sum += env.transition_model[state][nextState][policy[state]] * agent.utility[nextState][0]
            agent.utility[state][0] = env.rewards[state] + agent.gamma * sum
        unchanged = True
        for state in range(len(env.states)):
            maxSum = 0
            recordSum = np.zeros(len(env.actions))
            for action in range(len(env.actions)):
                for nextState in range(len(env.states)):
                    recordSum[action] += env.transition_model[state][nextState][action] * agent.utility[nextState][0]
            for nextState in range(len(env.states)):
                maxSum += env.transition_model[state][nextState][policy[state]] * agent.utility[nextState][0]
            if np.max(recordSum) > maxSum:
                policy[state] = np.argmax(recordSum)
                unchanged = False
        if unchanged:
            return policy
    ## END: Student code
