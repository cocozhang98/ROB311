...
def value_iteration(env: mdp_env, agent: mdp_agent, eps: float, max_iter = 1000) -> np.ndarray:
    ...
    for i in range(max_iter):
        delta = 0
        uPrime = np.copy(agent.utility)
        for state in range(len(env.states)):
            record_sum = np.zeros(len(env.actions))
            for action in range(len(env.actions)):
                for nextState in range(len(env.states)):
                    record_sum[action] += env.transition_model[state][nextState][action]*agent.utility[nextState][0]
            policy[state] = np.argmax(record_sum)
            agent.utility[state] = env.rewards[state] + agent.gamma * record_sum[policy[state]]
            delta = max(delta, np.abs(agent.utility[state][0] - uPrime[state][0]))
